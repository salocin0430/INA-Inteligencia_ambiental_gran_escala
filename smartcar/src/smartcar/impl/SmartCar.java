...
public class SmartCar {
	...
	protected SmartCar_AWSShadowPublisher awsShadowPublisher = null; // Publisher para AWS IoT Device Shadow
	protected SmartCar_AWSShadowSubscriber awsShadowSubscriber = null; // Subscriber para AWS IoT Device Shadow
	protected SmartCar_AWSAlertsPublisher awsAlertsPublisher = null; // Publisher para alertas en AWS IoT
	protected SmartCar_AWSInfoSubscriber awsInfoSubscriber = null; // Subscriber para canal 'info' en AWS IoT
	...
	// Cache de velocidad actual calculada (para AWS Shadow)
	protected int lastCalculatedSpeed = 0;
	
	// Flag para trackear si ya se disparó la alerta de llegada al destino
	protected boolean destinationAlertFired = false;
	...
	public SmartCar(String id) {
		...
		// Inicializar AWS IoT Device Shadow (opcional - solo si se configura thingName)
		// Se inicializará cuando se llame a enableAWSShadow()
	...
	public String getCurrentRoadSegment() {
	...
	/**
	 * Obtiene la velocidad actual calculada (sin calcularla de nuevo).
	 * Retorna la última velocidad calculada en el paso de simulación anterior.
	 * Si no se ha calculado aún, retorna la velocidad de crucero.
	 */
	public int getCurrentSpeed() {
		// Retornar la velocidad calculada en el último paso
		// Si no se ha calculado aún, retornar la velocidad de crucero
		return (this.lastCalculatedSpeed > 0) ? this.lastCalculatedSpeed : this.cruiserSpeed;
	}
	...
	public void setRoute(IRoute route) {
		...
		// Resetear flag de alerta de destino cuando se asigna una nueva ruta
		this.destinationAlertFired = false;
		...
		if (initialPos != null) {
			...
			// Suscribirse al topic de info del segmento inicial (simulador MQTT)
			...
			// Suscribirse al topic de info en AWS IoT (si está habilitado)
			if (this.awsInfoSubscriber != null && this.awsInfoSubscriber.isConnected()) {
				this.awsInfoSubscriber.subscribeToSegment(this.currentRoadSegment);
			}
		}
	}
	...
	public void onSimulationStep() {
		if (this.navigator == null) {
			return; // No hay Navigator
		}
		
		// Si llegó al destino, verificar si hay que disparar alerta y salir
		if (!this.navigator.isRouting()) {
			// Verificar si llegó al destino y disparar alerta de accidente si no se ha disparado
			if (this.navigator.getNavigatorStatus() == ina.vehicle.navigation.types.ENavigatorStatus.REACHED_DESTINATION && !this.destinationAlertFired) {
				System.out.println("(SmartCar: " + this.smartCarID + ") DESTINO ALCANZADO - Disparando alerta de accidente");
				this.notifyIncident("accidente");
				this.destinationAlertFired = true;
			}
			return; // No está en modo routing (puede estar en REACHED_DESTINATION)
		}
		
		// Calcular velocidad actual (respetando límites, señales, etc.)
		int velocidadActual = calcularVelocidadActual();
		this.lastCalculatedSpeed = velocidadActual; // Cachear para AWS Shadow
		...
		if (!nuevoSegmento.equals(this.currentRoadSegment)) {
			...
			// Actualizar suscripción a info en AWS IoT
			if (this.awsInfoSubscriber != null && this.awsInfoSubscriber.isConnected()) {
				this.awsInfoSubscriber.subscribeToSegment(nuevoSegmento);
			}
		...
		// Publicar estado en AWS IoT Device Shadow (si está habilitado)
		if (this.awsShadowPublisher != null && this.awsShadowPublisher.isConnected()) {
			this.awsShadowPublisher.publishState();
		}
	}
	...
	public void notifyIncident(String incidentType) {
		// Publicar en MQTT del simulador (existente)
		if ( this.notifier != null ) {
			this.notifier.alert(this.getSmartCarID(), incidentType, this.getCurrentPlace());
		}
		
		// Publicar también en AWS IoT 
		if ( this.awsAlertsPublisher != null && this.awsAlertsPublisher.isConnected() ) {
			this.awsAlertsPublisher.publishAlert(incidentType, this.currentRoadSegment, this.currentPosition);
		}
	}
	
	/**
	 * Habilita la integración con AWS IoT Device Shadow y canales adicionales.
	...
	public void enableAWSShadow(String thingName) {
		...
		if (this.awsShadowPublisher == null) {
			this.awsShadowPublisher = new SmartCar_AWSShadowPublisher(this, thingName);
			this.awsShadowPublisher.connect();
		}
		// Device Shadow: Recibir comandos
		if (this.awsShadowSubscriber == null) {
			this.awsShadowSubscriber = new SmartCar_AWSShadowSubscriber(this, thingName, "");
			this.awsShadowSubscriber.connect();
		}
		// Canal 'alerts' en AWS IoT: Publicar alertas
		if (this.awsAlertsPublisher == null) {
			this.awsAlertsPublisher = new SmartCar_AWSAlertsPublisher(this, thingName);
			this.awsAlertsPublisher.connect();
		}
		// Canal 'info' en AWS IoT: Escuchar información y alertas
		if (this.awsInfoSubscriber == null) {
			this.awsInfoSubscriber = new SmartCar_AWSInfoSubscriber(this, thingName, "");
			this.awsInfoSubscriber.connect();
		}
		System.out.println("(SmartCar: " + this.smartCarID + ") AWS IoT integration enabled for thing: " + thingName);
		System.out.println("  - Device Shadow: enabled");
		System.out.println("  - Alerts channel: enabled");
		System.out.println("  - Info channel: enabled");
	}
	
	/**
	 * Desconecta todos los componentes MQTT y AWS IoT
	 */
	public void disconnect() {
		...
		if (this.awsAlertsPublisher != null) {
			this.awsAlertsPublisher.disconnect();
		}
		if (this.awsInfoSubscriber != null) {
			this.awsInfoSubscriber.disconnect();
		}
		System.out.println("(SmartCar: " + this.smartCarID + ") All connections disconnected");
	}
	
	/**
	 * Limpia las señales de tráfico de un segmento (cuando el vehículo sale del segmento)
	 */
	protected void clearSignalsForSegment(String segment) {
		this.speedLimitSignals.remove(segment);
		this.trafficLightSignals.remove(segment);
	}
	...
	/**
	 * Crea un SmartCar configurado como Ambulancia.
	 * Las ambulancias ignoran límites de velocidad y semáforos.
	 * 
	 * @param id Identificador único del vehículo
	 * @param cruiserSpeed Velocidad de crucero (km/h)
	 * @return SmartCar configurado como ambulancia
	 */
	public static SmartCar createAmbulance(String id, int cruiserSpeed) {
		SmartCar ambulance = new SmartCar(id);
		ambulance.setVehicleRole("Ambulance");
		ambulance.setCruiserSpeed(cruiserSpeed);
		ambulance.setIgnoreLimits(true); // Ignora todos los límites
		System.out.println("(SmartCar) Created Ambulance: " + id + " with cruiser speed: " + cruiserSpeed + " km/h");
		return ambulance;
	}
	
	/**
	 * Crea un SmartCar configurado como Ambulancia con ruta.
	 * 
	 * @param id Identificador único del vehículo
	 * @param cruiserSpeed Velocidad de crucero (km/h)
	 * @param route Ruta a seguir
	 * @return SmartCar configurado como ambulancia con ruta asignada
	 */
	public static SmartCar createAmbulance(String id, int cruiserSpeed, IRoute route) {
		SmartCar ambulance = createAmbulance(id, cruiserSpeed);
		if (route != null) {
	...
	/**
	 * Crea un SmartCar configurado como Vehículo de Policía.
	 * Los vehículos de policía ignoran límites de velocidad y semáforos.
	 * 
	 * @param id Identificador único del vehículo
	 * @param cruiserSpeed Velocidad de crucero (km/h)
	 * @return SmartCar configurado como policía
	 */
	public static SmartCar createPolice(String id, int cruiserSpeed) {
		SmartCar police = new SmartCar(id);
		police.setVehicleRole("Police");
		police.setCruiserSpeed(cruiserSpeed);
		police.setIgnoreLimits(true); // Ignora todos los límites
		System.out.println("(SmartCar) Created Police vehicle: " + id + " with cruiser speed: " + cruiserSpeed + " km/h");
		return police;
	}
	
	/**
	 * Crea un SmartCar configurado como Vehículo de Policía con ruta.
	 * 
	 * @param id Identificador único del vehículo
	 * @param cruiserSpeed Velocidad de crucero (km/h)
	 * @param route Ruta a seguir
	 * @return SmartCar configurado como policía con ruta asignada
	 */
	public static SmartCar createPolice(String id, int cruiserSpeed, IRoute route) {
		SmartCar police = createPolice(id, cruiserSpeed);
		if (route != null) {
			police.setRoute(route);
		}
		return police;
	}
