...
public class SmartCar {
    ...
    public void initAWS(String endpoint, String thingName, String certPath, String keyPath, String caPath) {
        ...
        try {
            ...
            // Suscribir a los updates del Shadow AWS
            this.awsManager.subscribeToShadowUpdates(message -> {
                String deltaJson = new String(message.getPayload());
                System.out.println("[AWS-DELTA-CAR] " + deltaJson);
                // Aquí procesar "desired" si existe para control remoto (si fuese necesario)
            });

            MySimpleLogger.info("SmartCar", "✓ AWS IoT inicializado para SmartCar: " + thingName);
        } catch (Exception e) {
            MySimpleLogger.error("SmartCar", "Error AWS init: " + e.getMessage());
        }
    }

    /**
     * PUBLICAR ESTADO EN SHADOW AWS
     */
    private void publishAwsState() {
        if (awsManager == null) return;
        try {
            JSONObject state = new JSONObject();
            state.put("vehicle_id", this.smartCarID);
            state.put("road_segment", this.currentRoadSegment);
            state.put("position", this.currentPosition);
            state.put("speed", this.cruiserSpeed);
            state.put("timestamp", System.currentTimeMillis());
            awsManager.updateDeviceShadow(state.toString());
        } catch (Exception e) {
            MySimpleLogger.error("SmartCar", "Error publish AWS: " + e.getMessage());
        }
    }

    // Métodos que actualizan posición y velocidad llamarán a publishAwsState:
    ...
    public void updateSpeed(int newSpeed) {
        this.cruiserSpeed = newSpeed;
        // lógica local aquí
        publishAwsState();
    }

    public void publishAlert(String message) {
        // lógica local alertas
        publishAwsState();
    }


    public void setSmartCarID(String smartCarID) { this.smartCarID = smartCarID; }
    public String getSmartCarID() { return smartCarID; }
    public String getVehicleRole() { return vehicleRole; }
    public void setVehicleRole(String vehicleRole) { this.vehicleRole = vehicleRole; }
    ...
    public boolean isIgnoreLimits() { return ignoreLimits; }
    public void setIgnoreLimits(boolean ignoreLimits) { this.ignoreLimits = ignoreLimits; }
